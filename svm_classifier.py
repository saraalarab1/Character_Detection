...
def save_model(eval_accuracy, test_score, conf_rep, for_ensemble, features, arabic):
    ...
    yaml_info['prediction_model'] = f"pretrained_svm_${model_language}_model.pkl"
    yaml_info['features'] = features
    yaml_info['training'] = 'completed'
    yaml_info['name'] = f'pretrained_svm_${model_language}model.pkl'
    ...
    with open(yaml_path, 'w') as output:
        ...
        if 'features' not in yaml_info:
            yaml_info = dict()
        if 'svm' not in yaml_info:
            yaml_info['svm'] = dict()
        yaml_info['svm'][model_language]['eval_accuracy'] = float(eval_accuracy)
        yaml_info['svm'][model_language]['test_score'] = float(test_score)
        yaml_info['svm'][model_language]['conf_rep'] = get_info(conf_rep)
        yaml_info['svm'][model_language]['weight'] = 1
