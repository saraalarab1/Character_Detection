...
def train(x, y, testing_size, model_version):
    ...
    X0_train = X0_train[0:int(len(X0_train)/10)]
    Y0_train = Y0_train[0:int(len(Y0_train)/10)]
    scaling = MinMaxScaler(feature_range=(-1,1)).fit(X0_train)
    pickle.dump(scaling, open(f"models/language_model/scaler.pkl", 'wb'))
    ...
    model_name = "pretrained_language_model.pkl"
    if model_version:
        pickle.dump(model, open(f"models/{model_version}/{model_name}", 'wb'))
    else:
        pickle.dump(model, open(f"models/language_model/{model_name}", 'wb'))
...
def test(X_test, Y_test,model_version):
    ...
    if model_version:
        ...
        model = pickle.load(open('models/language_model/pretrained_language_model.pkl', 'rb' ))
...
def train_language_model(features, model_version=None):
    print('training')
...
# train_language_model(['nb_of_pixels_per_segment', 'horizontal_line_intersection', 'vertical_line_intersection'])
